...
# Output that will be compared must be printed to stdout
 ...
 pgrep -af "qemu-system.*" 2>&1 && echo q | nc -N 127.0.0.1 55555  >> "$tempdir"/$qemuoutputfile 2>&1
 
 # Ask a bit more firmly with SIGTERM
 pgrep -af "qemu-system.*" 2>&1 && kill $qemupid 2>&1
 
 pgrep -af "qemu-system.*" 2>&1 && sleep 0.1s 2>&1
 
 # Time to die
 pgrep -af "qemu-system.*" 2>&1 && kill -9 $qemupid 2>&1
